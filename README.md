...
**说明扩展已经生效**。当你开启扩展后，B 站不知道你是谁，只能根据他能获取的其他数据（比如你的访问IP，你用的浏览器）计算出一套‘热门清单’给你推送。当你浏览时，浏览器内会有一个临时标记，记录已经给你推送过什么，避免重复推送，但只要你关了浏览器，这个临时标记就会被清除，你再访问 B 站，B 站又‘彻底忘了你是谁’，而你其他情况数据变化不大，而且这么短时间内，全站热门内容还是那些，所以 B 站计算出的‘热门清单’自然还是（几乎）一模一样的。
